define(
	...
	function(q, Sg)
	{
		...
		function StartNode(ctx, ipp, appNodeId, handler)
		{
			return new Promise((resolve, reject)=>{
				ctx.obj = new Sg.Client(ipp, appNodeId);
				ctx.resolve = resolve;
				ctx.reject = reject;
				...
				//如果3秒未连接上，则reject
				setTimeout(()=>{
					reject('连接超时,' + ipp + '/' + appNodeId + '，请确认路由服务已经启动。');
				}, 3000); 			});
		}
		//结束一个app节点，直接在ctx上进行
		...
		function SendData(ctx1, ctx2, appHeader, appData, additionalDestAddrs)
		{
			return new Promise((resolve, reject)=>{
				...
				setTimeout(()=>{ reject('超时没有接收到数据'); }, 100); //100ms超时
			});
		}
		//需要发送的数据的app头
		var appHeader = {
